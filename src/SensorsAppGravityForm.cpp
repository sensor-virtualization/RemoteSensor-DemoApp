...
SensorsAppGravityForm::SensorsAppGravityForm(void)
	: __sensorMgr()
{
...
bool
SensorsAppGravityForm::Initialize(void)
{
	int ScreenHeight;
	...
	SetName(L"GravitySensorForm");
	pHeader->SetTitleText(L"Gravity");
...
result
SensorsAppGravityForm::OnInitializing(void)
{
	__sensorMgr.Construct();

	/* Check for the Sensors is supported in the current device or not */
	if (!__sensorMgr.IsAvailable(SENSOR_TYPE_GRAVITY))
	{
...
SensorsAppGravityForm::CreateLabel(Rectangle& rect, Color textcolor, Color background)
{
	Tizen::Ui::Controls::Label* _pLabel;
...
result
SensorsAppGravityForm::OnTerminating(void)
{
	StopSensor();
	return E_SUCCESS;
}
...
void
SensorsAppGravityForm::OnFormBackRequested(Tizen::Ui::Controls::Form& source)
{
	SceneManager* pSceneManager = SceneManager::GetInstance();
	AppAssert(pSceneManager);
	AppAssert(pSceneManager->GoBackward(BackwardSceneTransition(MainScene)) == E_SUCCESS);
...
void
SensorsAppGravityForm::OnSceneActivatedN(const Tizen::Ui::Scenes::SceneId& previousSceneId,
...
void
SensorsAppGravityForm::OnSceneDeactivated(const Tizen::Ui::Scenes::SceneId& currentSceneId,
...
result
SensorsAppGravityForm::OnDraw(void)
{
...
void
SensorsAppGravityForm::OnDataReceived(SensorType sensorType, SensorData& sensorData, result r)
{
...
void
SensorsAppGravityForm::StartSensor(void)
{
	long interval = 0L;
...
void
SensorsAppGravityForm::StopSensor(void)
{

	AppLog("Deactivating SensorsAppGravityForm...");
