...
SensorsAppTiltForm::SensorsAppTiltForm(void)
	: __sensorMgr()
{
...
bool
SensorsAppTiltForm::Initialize(void)
{
	int ScreenHeight;
	...
	SetName(L"TiltSensorForm");
	pHeader->SetTitleText(L"Tilt");
...
/* Create all the labels to display the values */
	...
	Rectangle Rectz(Point(200, 240 / __scaleValue), Dimension(225, 50 / __scaleValue));
	_pLabel = CreateLabel(Rectz, Color::GetColor(COLOR_ID_BLACK), Color::GetColor(COLOR_ID_WHITE));
	_pLabel->SetText(L"Azimuth:");
	__pScrollPanel->AddControl(_pLabel);
   /* Create all the labels to display the values */
...
SensorsAppTiltForm::CreateLabel(Rectangle& rect, Color textcolor, Color background)
{
	Tizen::Ui::Controls::Label* _pLabel;
...
result
SensorsAppTiltForm::OnTerminating(void)
{
	StopSensor();
	return E_SUCCESS;
}
...
void
SensorsAppTiltForm::OnFormBackRequested(Tizen::Ui::Controls::Form& source)
{
	SceneManager* pSceneManager = SceneManager::GetInstance();
	AppAssert(pSceneManager);
	AppAssert(pSceneManager->GoBackward(BackwardSceneTransition(MainScene)) == E_SUCCESS);
...
void
SensorsAppTiltForm::OnSceneActivatedN(const Tizen::Ui::Scenes::SceneId& previousSceneId,
...
void
SensorsAppTiltForm::OnSceneDeactivated(const Tizen::Ui::Scenes::SceneId& currentSceneId,
...
result
SensorsAppTiltForm::OnDraw(void)
{
...
void
SensorsAppTiltForm::OnDataReceived(SensorType sensorType, SensorData& sensorData, result r)
{
	String strX, strY, strZ;

	__pCanvas->Clear();

	TiltSensorData& data = static_cast<TiltSensorData&>(sensorData);
	AppLog("Tilt SensorData    x = %5.4f , y = %5.4f,  z = %5.4f ", data.azimuth,data.pitch,data.roll);
	...
	if(data.azimuth > 0)
	__pCanvas->DrawArc(RectRoll, 0, data.azimuth, ARC_STYLE_FILLED_PIE);
	else
	__pCanvas->DrawArc(RectRoll, data.azimuth, 0, ARC_STYLE_FILLED_PIE);

	Invalidate(true);
}

void
SensorsAppTiltForm::StartSensor(void)
{
	long interval = 0L;

	AppLog("Activating ...");

	__sensorMgr.GetMinInterval(SENSOR_TYPE_TILT, interval);
...
void
SensorsAppTiltForm::StopSensor(void)
{

	AppLog("Deactivating SensorsAppTiltForm...");
