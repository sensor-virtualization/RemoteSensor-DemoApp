...
SensorsAppRemoteGyroForm::SensorsAppRemoteGyroForm(void)
	: __sensorMgr()
{
...
bool
SensorsAppRemoteGyroForm::Initialize(void)
{
	int ScreenHeight;
	...
	SetName(L"RemoteGyro");
	pHeader->SetTitleText(L"RemoteGyro");
...
result
SensorsAppRemoteGyroForm::OnInitializing(void)
{
...
SensorsAppRemoteGyroForm::CreateLabel(Rectangle& rect, Color textcolor, Color background)
{
	Tizen::Ui::Controls::Label* _pLabel;
...
result
SensorsAppRemoteGyroForm::OnTerminating(void)
{
	StopSensor();
	return E_SUCCESS;
}
...
void
SensorsAppRemoteGyroForm::OnFormBackRequested(Tizen::Ui::Controls::Form& source)
{
	SceneManager* pSceneManager = SceneManager::GetInstance();
	AppAssert(pSceneManager);
	AppAssert(pSceneManager->GoBackward(BackwardSceneTransition(MainScene)) == E_SUCCESS);
...
void
SensorsAppRemoteGyroForm::OnSceneActivatedN(const Tizen::Ui::Scenes::SceneId& previousSceneId,
...
void
SensorsAppRemoteGyroForm::OnSceneDeactivated(const Tizen::Ui::Scenes::SceneId& currentSceneId,
...
result
SensorsAppRemoteGyroForm::OnDraw(void)
{
...
void
SensorsAppRemoteGyroForm::OnDataReceived(SensorType sensorType, SensorData& sensorData, result r)
{
	...
	if(data.y > 0)
	__pCanvas->DrawArc(RectArc, 0, data.y, ARC_STYLE_FILLED_PIE);
	else
	__pCanvas->DrawArc(RectArc, data.y, 0, ARC_STYLE_FILLED_PIE);
	...
	if(data.x > 0)
	__pCanvas->DrawArc(RectPitch, 0, data.x, ARC_STYLE_FILLED_PIE);
	else
	__pCanvas->DrawArc(RectPitch, data.x, 0, ARC_STYLE_FILLED_PIE);
	...
	__pCanvas->SetForegroundColor(Color::GetColor(COLOR_ID_GREY));
	Rectangle RectRoll( Point( (GetClientAreaBounds().x + PosBaseX - 200), GetClientAreaBounds().y + 450 + 25 ), Dimension(125, 125) );
	if(data.z > 0)
	__pCanvas->DrawArc(RectRoll, 0, data.z, ARC_STYLE_FILLED_PIE);
	else
	__pCanvas->DrawArc(RectRoll, data.z, 0, ARC_STYLE_FILLED_PIE);
...
void
SensorsAppRemoteGyroForm::StartSensor(void)
{
	long interval = 0L;
...
void
SensorsAppRemoteGyroForm::StopSensor(void)
{
