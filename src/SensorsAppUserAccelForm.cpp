...
SensorsAppUserAccelForm::SensorsAppUserAccelForm(void)
	: __sensorMgr()
{
...
bool
SensorsAppUserAccelForm::Initialize(void)
{
	int ScreenHeight;
	...
	SetName(L"UserAccelSensorForm");
	pHeader->SetTitleText(L"User Acceleration");
...
result
SensorsAppUserAccelForm::OnInitializing(void)
{
	__sensorMgr.Construct();

	/* Check for the Sensors is supported in the current device or not */
	if (!__sensorMgr.IsAvailable(SENSOR_TYPE_USER_ACCELERATION))
	{
		AppLog("[Fail] SENSOR_TYPE_USER_ACCELERATION Sensor type does not supported. This sample cannot be activated.");
	...
	Rectangle RectRange(Point(0,  15), Dimension(300, 50/ __scaleValue));
...
SensorsAppUserAccelForm::CreateLabel(Rectangle& rect, Color textcolor, Color background)
{
	Tizen::Ui::Controls::Label* _pLabel;
...
result
SensorsAppUserAccelForm::OnTerminating(void)
{
	StopSensor();
	return E_SUCCESS;
}
...
void
SensorsAppUserAccelForm::OnFormBackRequested(Tizen::Ui::Controls::Form& source)
{
	SceneManager* pSceneManager = SceneManager::GetInstance();
	AppAssert(pSceneManager);
	AppAssert(pSceneManager->GoBackward(BackwardSceneTransition(MainScene)) == E_SUCCESS);
...
void
SensorsAppUserAccelForm::OnSceneActivatedN(const Tizen::Ui::Scenes::SceneId& previousSceneId,
...
void
SensorsAppUserAccelForm::OnSceneDeactivated(const Tizen::Ui::Scenes::SceneId& currentSceneId,
...
result
SensorsAppUserAccelForm::OnDraw(void)
{
...
void
SensorsAppUserAccelForm::OnDataReceived(SensorType sensorType, SensorData& sensorData, result r)
{
	...
	UserAccelerationSensorData& data = static_cast<UserAccelerationSensorData&>(sensorData);
	AppLog(" UserAccelerationSensorData    x = %5.4f , y = %5.4f,  z = %5.4f ", data.x,data.y,data.z);
...
void
SensorsAppUserAccelForm::StartSensor(void)
{
	long interval = 0L;

	__sensorMgr.GetMinInterval(SENSOR_TYPE_USER_ACCELERATION, interval);
	if (interval < 50)
	{
		interval = 50;
	}

	__sensorMgr.AddSensorListener(*this, SENSOR_TYPE_USER_ACCELERATION, interval, false);
}

void
SensorsAppUserAccelForm::StopSensor(void)
{
	AppLog("Removing the listener SensorsAppUserAccelForm...");
