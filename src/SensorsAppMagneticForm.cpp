...
SensorsAppMagneticForm::SensorsAppMagneticForm(void)
	: __sensorMgr()
{
...
bool
SensorsAppMagneticForm::Initialize(void)
{
	int ScreenHeight;
	...
	SetName(L"MagneticSensorForm");
	pHeader->SetTitleText(L"Magnetic");
...
result
SensorsAppMagneticForm::OnInitializing(void)
{
	...
	{
		AppLog("[Fail] SENSOR_TYPE_MAGNETIC Sensor type does not supported. This sample cannot be activated.");
	...
	Rectangle RectRange(Point(0,  0), Dimension(400, 50/ __scaleValue));
	_pLabel = CreateLabel(RectRange, Color::GetColor(COLOR_ID_BLACK), Color::GetColor(COLOR_ID_WHITE));
	_pLabel->SetText(L"Value Range: -2000 µT ~ +2000 µT");
	...
	Rectangle Rectxyz(Point(115, 50/ __scaleValue), Dimension(275, 50/ __scaleValue));
	_pLabel = CreateLabel(Rectxyz, Color::GetColor(COLOR_ID_BLACK), Color::GetColor(COLOR_ID_WHITE));
	_pLabel->SetText(L"Strength:");
	__pScrollPanel->AddControl(_pLabel);
	Rectangle RectXVH(Point(400, 50/ __scaleValue), Dimension(210, 50/ __scaleValue));
	__pLabelXYZ = CreateLabel(RectXVH, Color::GetColor(COLOR_ID_BLACK), Color::GetColor(COLOR_ID_GREY));
	__pScrollPanel->AddControl(__pLabelXYZ);

	Rectangle RectX(Point(125, 120 / __scaleValue), Dimension(75, 50 / __scaleValue));
	_pLabel = CreateLabel(RectX, Color::GetColor(COLOR_ID_BLACK), Color::GetColor(COLOR_ID_RED));
	__pScrollPanel->AddControl(_pLabel);
	Rectangle RectXV(Point(400, 120 / __scaleValue), Dimension(210, 50 / __scaleValue));
	...
	Rectangle Rectx(Point(315, 120 / __scaleValue), Dimension(75, 50 / __scaleValue));
	_pLabel = CreateLabel(Rectx, Color::GetColor(COLOR_ID_BLACK), Color::GetColor(COLOR_ID_WHITE));
	_pLabel->SetText(L"X:");
	__pScrollPanel->AddControl(_pLabel);

	Rectangle RectY(Point(125, 190 / __scaleValue), Dimension(75, 50 / __scaleValue));
	_pLabel = CreateLabel(RectY, Color::GetColor(COLOR_ID_BLACK), Color::GetColor(COLOR_ID_BLUE));
	__pScrollPanel->AddControl(_pLabel);
	Rectangle RectYV(Point(400, 190 / __scaleValue), Dimension(210, 50 / __scaleValue));
	...
	Rectangle Recty(Point(315, 190 / __scaleValue), Dimension(75, 50 / __scaleValue));
	_pLabel = CreateLabel(Recty, Color::GetColor(COLOR_ID_BLACK), Color::GetColor(COLOR_ID_WHITE));
	_pLabel->SetText(L"Y:");
	__pScrollPanel->AddControl(_pLabel);
	Rectangle RectZ(Point(125, 260 / __scaleValue), Dimension(75, 50 / __scaleValue));
	_pLabel = CreateLabel(RectZ, Color::GetColor(COLOR_ID_BLACK), Color::GetColor(COLOR_ID_GREEN));
	__pScrollPanel->AddControl(_pLabel);
	Rectangle RectZV(Point(400, 260 / __scaleValue), Dimension(210, 50 / __scaleValue));
	...
	Rectangle Rectz(Point(315, 260 / __scaleValue), Dimension(75, 50 / __scaleValue));
...
SensorsAppMagneticForm::CreateLabel(Rectangle& rect, Color textcolor, Color background)
{
	Tizen::Ui::Controls::Label* _pLabel;
...
result
SensorsAppMagneticForm::OnTerminating(void)
{
	StopSensor();
	return E_SUCCESS;
}
...
void
SensorsAppMagneticForm::OnFormBackRequested(Tizen::Ui::Controls::Form& source)
{
	SceneManager* pSceneManager = SceneManager::GetInstance();
	AppAssert(pSceneManager);
	AppAssert(pSceneManager->GoBackward(BackwardSceneTransition(MainScene)) == E_SUCCESS);
...
void
SensorsAppMagneticForm::OnSceneActivatedN(const Tizen::Ui::Scenes::SceneId& previousSceneId,
...
void
SensorsAppMagneticForm::OnSceneDeactivated(const Tizen::Ui::Scenes::SceneId& currentSceneId,
...
result
SensorsAppMagneticForm::OnDraw(void)
{
...
void
SensorsAppMagneticForm::OnDataReceived(SensorType sensorType, SensorData& sensorData, result r)
{
	float x = 0.0F , y = 0.0F, z = 0.0F;
	String strX, strY, strZ, StrStrength;
	bool xpos, ypos, zpos;
	...
	AppLog("Magnetic SensorData    x = %5.4f , y = %5.4f,  z = %5.4f ", data.x,data.y,data.z);
	...
	StrStrength.Append(sqrt(data.x * data.x + data.y * data.y + data.z * data.z));
	StrStrength.SetLength(StrStrength.GetLength());
	__pLabelXYZ->SetText(StrStrength + "  ");
...
void
SensorsAppMagneticForm::StartSensor(void)
{
	long interval = 0l;

	AppLog("Adding the listener SensorsAppMagneticForm...");

	__sensorMgr.GetMinInterval(SENSOR_TYPE_MAGNETIC, interval);
	if (interval < 50)
	{
		interval = 50;
	}

	__sensorMgr.AddSensorListener(*this, SENSOR_TYPE_MAGNETIC, interval, false);

}

void
SensorsAppMagneticForm::StopSensor(void)
{
	AppLog("Removing the listener SensorsAppMagneticForm...");
